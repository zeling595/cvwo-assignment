...
class TodosContainer extends Component {
  ...
  createTodo = (e) => {
	if (e.key === 'Enter') {
	  axios.post('http://localhost:3001/api/v1/todos', {todo: {title: e.target.value}})
	  .then(response => {
  ...
  createCategory = (e) => {
	if (e.key === 'Enter') {
		axios.post('http://localhost:3001/api/v1/todos', {todo: {category: e.target.value}})
		.then(response => {
		  const todos = update(this.state.todos, {
			$splice: [[0, 0, response.data]]
		  })
      ...
      <div>
		...
		<div className="inputContainer">
			<input className="taskInput" type="text" 
				...
				onKeyPress={this.createCategory} 
